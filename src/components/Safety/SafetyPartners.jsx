...
const SafetyPartners = () => {
    return (
        <div className="bg-[#1a2532]  flex flex-col items-center justify-center p-6">
            <h2 className="text-white text-3xl font-bold mb-10">Our Safety Partners</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-10 w-full max-w-7xl">
                {partners.map(partner => (
                    <div
                        key={partner.id}
                        className="bg-[#3d4755] rounded-lg p-4 flex flex-col items-center justify-center shadow-md hover:shadow-xl transition"
                    >
                        <img
                            src={partner.image}
                            alt={partner.name}
