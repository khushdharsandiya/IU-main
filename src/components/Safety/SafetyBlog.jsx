    ...
    <div className="bg-[#0f172a] py-16 px-6 text-white">
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-10">
          ...
          <button className="bg-[#1e293b] px-4 py-2 rounded-md text-sm transition hover:bg-green-900">
            View All Articles
          </button>
        </div>
        <div className="grid md:grid-cols-3 gap-10">
          {blogs.map((blog) => (
            <div
              key={blog.id}
              className="bg-gray-950 rounded-lg overflow-hidden shadow-md transform transition duration-300 hover:scale-105"
            >
              {blog.image && (
                <img
                  src={blog.image}
              ...
              <div className="p-5">
                ...
                <p className="text-sm text-gray-300 mb-4">{blog.desc}</p>
                <Link
                  href="#"
                  onClick={(e) => {
                    e.preventDefault();
